...
class IsTeacherOrAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return bool(
            (request.user and request.user.role == 'teacher') or (request.user and request.user.role == 'admin')
        )
...
class TeacherAttendanceOwnerOrAdmin(BasePermission):
    """
    Allows access to object owners (teachers) or admins.
    """
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # Teacher ownership check
        is_teacher_owner = bool(
            request.user and request.user.role == 'teacher' and obj.subject.teacher.user == request.user
        )
